...
- **ICA `assets/报告模板与版式.md`** — 中式四章结构、封面信息项、体量基准**沿用**;但"封面双页(DOCX 分页)"改成 Obsidian 开头门面区块(见 §二)
...
门面三框是封面级浓缩框(回答"是什么/值多少/凭什么"),不是"把表格当独立 H2 章节",也不是"12 行技术面板大表灌进叙述"。数字接 VM(见 [估值章节数据接入.md](./估值章节数据接入.md))。门面之后正文直接从"一、公司概况"起(ICA 不写独立引言段)。
...
- ❌ 流程性元话语("本节由 agent 自动生成" / "数据生成于 YYYY" / "脚本路径" / "源数据见…":过程过程说明不进交付物)
- ❌ 把表格当独立 H2 章节(门面三框除外)
