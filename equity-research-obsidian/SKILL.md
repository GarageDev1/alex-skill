...
这个 skill 是 [`initial-coverage-advanced`](../initial-coverage-advanced)(ICA)的变体——研报的**正文结构 / 写作规范 / 体量 / 质检**直接沿用 ICA,只换两处:**接 valuation-model 做估值**(替代 ICA 阶段 2/3 独立建模)+ **Obsidian md + 全 inline ECharts**(替代 ICA 的 DOCX + PNG)。
...
这个 skill 沿用 ICA 的写作/结构/质检,自写估值接法/图表/格式三处。**不要开工就把下面的文件全 Read**——执行到哪步,才读那步标 `[步N]` 的文件;那一步的上下文不背其余步的细节。全程必备的只有下方《核心原则》+《执行工序》两节。
...
## 核心铁律摘要(指令遵循底线,即使漏 Read 上方文件也要守)
...
**唯一停顿 = 估值定稿前**:模型跑完、第四章估值与目标价/评级将定时,停一次让用户 review 估值假设与结论——估值是报告命门,这是整个管线唯一的强制停顿。valuation-model 选物理锚出现歧义时(异常路径),也允许停一次。其余连续推进。
