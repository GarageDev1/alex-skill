...
2. 修改文字：编辑 [assets/brand.json](assets/brand.json)。`masthead_text` 控制左上角；`footer_title`、`footer_intro` 与 `footer_points` 控制底部介绍区；`qr_label` 同时控制顶部与底部的“扫码加入交流群”说明。
...
| `footer_points` | 底部左侧 | “人 / 事 / 钱”三项服务说明 |
| `qr_image` | 顶部右侧交流群入口、底部右侧 | 同一张二维码资产路径 |
| `qr_label` | 两处二维码下方 | 扫码加入交流群说明 |
...
渲染后确认：顶部交流群二维码和底部二维码图案相同、两处均显示“扫码加入交流群”、左上角品牌短句正确、底部“人 / 事 / 钱”文案完整可读；真实二维码还应实测扫码。
