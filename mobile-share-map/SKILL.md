...
将长文档转为适合手机阅读和转发的 1080px 宽竖向长图。默认使用简体中文。
...
| 1 | `steps/01-read-source.md` | 读取源文档，建立论证骨架 |
| 2 | `steps/02-output-policy.md` | 判断并记录 single/dual 输出策略 |
| 3 | `steps/03-create-json.md` | 按 schema 创建结构化 JSON |
| 3a | `steps/03a-audit-json.md` | **Sub-agent** — JSON 合规审计，建议性 |
| 4 | `steps/04-thumbnails.md` | 生成研报页面缩略图 |
| 5 | `steps/05-render.md` | 运行渲染脚本生成 HTML/PNG |
| 6 | `steps/06-verify.md` | 自动化验收 + 目视确认 |
...
- 运行任何 Python 脚本前，先设置 `PYTHONUTF8=1`（PowerShell: `$env:PYTHONUTF8="1"`，Bash: `export PYTHONUTF8=1`），否则中文内容会因系统默认编码变为乱码。
