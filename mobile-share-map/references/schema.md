...
传导链示例：
- HBM扩产 → 存储芯片供需 → 内存/硬盘价格 → **手机、电脑贵不贵**
- AI芯片竞争 → 云计算成本 → **你用的AI产品定价**
- 光伏产能过剩 → 组件价格战 → **家庭装光伏划不划算**
- 航运运力紧张 → 运费上涨 → **海淘和进口商品涨价**

副标题不需要走完整条链，只需比源文档的行业视角再推一步，落到读者能感知的层面。

- 好：`产能结构与扩产进度一图看清——这轮建厂潮决定明年内存条和手机存储贵不贵`
...
除非用户显式要求省略，否则必须生成缩略图并在 JSON 中加入 `docx_preview` 字段，在正文与底栏之间插入研报页面缩略图预览横条：
