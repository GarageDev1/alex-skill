...
px = K.write_price_chart(wb.create_sheet(S_PX), MONTHLY, {
    "fn": phase_fn,
    "rows": [("① 利率杀估值", "2022 加息周期，高估值 SaaS 普跌"),
...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "tracking": {
        ...
        "rows": [
            ...
            ("NRR（美元净留存率）", "125%", "命门：存量扩张乘数，>120% 强", "季报披露", "跌破 118% → 全链下调、转 Bear"),
            ("NowAssist ARR/占比", "5% 占比 +50% 增速", "命门：第二曲线兑现", "季报 segment 披露", "增速跌破 30% → 下调 g_ai"),
            ("__band__", "二、利润率"),
            ("GAAP 营业利润率", "13.7%", "命门：AI 投入 vs 运营杠杆", "季报", "环比转跌 → 下调 opm 路径"),
            ("__band__", "三、估值与利率"),
            ("forward P/E", "~48x", "命门：高估值对利率敏感", "股价/一致预期", "10Y 突破 4.8% → P/E 目标下调"),
        ],
    },
})
