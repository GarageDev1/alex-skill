    ...
    K.write_consensus(wb.create_sheet(S_CONS), {
        ...
        "assumptions": [
            ...
            ("储备收益率\n(利率路径)", "EFFR 已从 5.33% 降至 3.63%（-170bps）；期货隐含继续小幅下行。",
             "降息幅度与节奏 = 收入端最大宏观变量（96% 收入敞口于短端利率）。", "Base 2026E 3.7%、2027E 3.3%，随美联储路径缓降后企稳 3.0%。"),
            ("Coinbase 分销分成", "FY2025 分销成本占储备收入 63%（$1.66B），付 Coinbase 约 $0.9B（占营收 54%）；协议据报 2026-08 续约。",
             "续约条款 + off-platform 份额能否上升 = 留存率命门。", "Base 分成比 62%→58%(2030)，假设 Circle 逐步分散分销、续约中性偏温和改善。"),
    ...
    sw = K.write_scenario_switch(wb.create_sheet(S_SW), {
        ...
        "levers": [
            ...
            {"key": "dist", "name": "分销分成比", "fmt": K.PCT, "cols": FCf,
             "vals": DISTR, "desc": "付 Coinbase 等的储备收入分成（命门）。FY2025 63%；协议 2026-08 续约。off-platform 份额越高分成越低。",
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "blocks": [
            {"title": "A. 基本面拐点 — 单位经济学在改善还是恶化？", "rows": [
                ("分销分成比走势", "FY2022 39% → FY2025 63%", "命门：留存率。分成比若续升 = 增长替 Coinbase 打工。", True),
                ("储备收益率(利率)", "EFFR 5.33%→3.63%，仍下行", "96% 收入敞口于短端利率；降息直接压收入。"),
        ...
        "tracking": {
            ...
            "rows": [
                ("__band__", "一、物理锚（量）"),
                ("USDC 流通量", "约 $73B（2026-07）", "命门：储备收入盘子", "DefiLlama/circle.com 月度 + 季报", "连续下滑或停滞 → 下调 circ 增速转 Bear"),
                ("__band__", "二、利率（价）"),
                ("储备收益率/EFFR", "3.63%（2026-07）", "命门：96% 收入敞口", "FRED EFFR + 季报收益率", "深降至 2.5% 以下 → 下调 yld 全链重算"),
                ("__band__", "三、分销分成（留存）"),
                ("Coinbase 分成比", "63%（FY2025），续约 2026-08", "命门：留存率", "季报 distribution costs / reserve income", "续约恶化升至 65%+ → 转 Bear"),
                ("__band__", "四、竞争威胁"),
                ("OUSD 联盟进展", "2026-06-30 启动，140+ 家", "命门：收益分给采用方冲击模式", "OUSD 采用量 + Circle 应对", "OUSD 上量 → 压 circ 份额 + 逼高 dist"),
            ],
        },
    })
