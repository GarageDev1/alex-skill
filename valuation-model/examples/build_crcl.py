    ...
    K.write_consensus(wb.create_sheet(S_CONS), {
        ...
        "assumptions": [
            ...
            ("储备收益率\n(利率路径)", "EFFR 已从 5.33% 降至 3.63%（-170bps）；期货隐含继续小幅下行。",
             "降息幅度与节奏 = 收入端最大宏观变量（96% 收入影响比例于短端利率）。", "Base 2026E 3.7%、2027E 3.3%，随美联储路径缓降后企稳 3.0%。"),
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "blocks": [
            {"title": "A. 基本面拐点 — 单位经济学在改善还是恶化？", "rows": [
                ...
                ("储备收益率(利率)", "EFFR 5.33%→3.63%，仍下行", "96% 收入影响比例于短端利率；降息直接压收入。"),
        ...
        "tracking": {
            ...
            "rows": [
                ...
                ("__band__", "二、利率（价）"),
                ("储备收益率/EFFR", "3.63%（2026-07）", "关键敏感项：96% 收入影响比例", "FRED EFFR + 季报收益率", "深降至 2.5% 以下 → 下调 yld 全链重算"),
                ("__band__", "三、分销分成（留存）"),
