...
def build():
    ...
    sw = K.write_scenario_switch(wb.create_sheet(S_SW), {
        ...
        "triggers": [
            ...
            ("Bull", "份额韧性(数据飞轮+交付效率壁垒守住)；单车费随 L3 显著上行；Robotaxi 2028 商业化兑现并显式计入；EV/Sales 维持高位。"),
        ],
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "tracking": {
            ...
            "rows": [
                ("__band__", "一、物理锚"),
                ("年度装车量 / 累计装机", "累计破 100 万台(2026 中)", "命门：NOA 渗透 × Momenta 份额", "CIC / 公司月度交付", "装车量增速掉档 → 转 Bear"),
                ("独立 Urban NOA 份额", "64.5%(2026-02 前 12 月)", "命门：数据飞轮/交付壁垒能否守住份额", "CIC 季度", "份额跌破 50% → 砍装车量路径"),
                ("__band__", "二、单车经济"),
                ("单车 License 费(反推)", "~RMB1,940(2025)", "命门：L3/Urban mix 抬价 vs 竞争压价", "License 收入÷装车量", "环比下滑 → 下调 fee 路径"),
                ("__band__", "三、盈利拐点"),
                ("经营利润 / 现金流", "经营亏损 -0.64B / OCF -0.28B(2025)", "命门：R&D 增速 < 收入增速", "季报", "R&D 增速反超收入 → 推迟转正"),
                ("__band__", "四、Robotaxi 期权"),
                ("Robotaxi 商业运营", "上海获无人化许可(2026-01)；仅 31 台", "命门：2028 商业化能否兑现", "公司/监管公告", "上海/Abu Dhabi 车队起量 → 计入 Base"),
                ("__band__", "五、供给"),
                ("基石解禁", "占发行 49.95%，6 个月锁定", "命门：2027-01 解禁供给冲击", "招股书锁定期", "解禁前后波动 → 交易层面留意"),
            ],
        },
    })
