...
K.write_consensus(wb.create_sheet(S_CONS), {
    ...
    "divergences": [
        ...
        "② ARR 质量: 共识把 ¥17亿 ARR 当确认收入的先行指标线性外推; 我们认为其中 Coding Plan 订阅对竞品能力追平高度敏感, 给 Bear 案 40% 折扣。",
    ],
    "stances": [
...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "tracking": {
        ...
        "rows": [
            ("__band__", "一、定价权与能力代差(MaaS 链命门)"),
            ("竞品 coding 基准", "GLM-5 国内领先", "命门: 单价假设(三次提价的根基)", "DeepSeek/Qwen 新版发布即测(SWE-bench 等)", "追平 → mprice 翻 Bear 路径"),
            ("MaaS ARR 增速", "¥17亿(+60x)", "命门: 份额×单价的总验证", "中报/年报披露", "<50% YoY → 收入链下修"),
            ("__band__", "二、筹码与资金事件"),
            ("解禁减持公告", "7/8 起", "命门: 情绪值路径", "联交所权益披露/大宗成交", "密集减持 → 情绪值下调 0.1-0.2"),
            ("A股发行隐含估值", "待申报", "命门: 估值重锚方向", "上交所审核公告", "<¥2,500亿 → 翻 Bear; 接近 H 股 → 上调 Bull 概率"),
            ("__band__", "三、生存线"),
            ("现金余额/烧钱", "¥22.59亿+38亿, 年烧 30-40亿", "命门: 持续经营(Bear 案 2027 见底)", "中报现金流量表", "A股 2027H1 前未落地 → 生存折价入模"),
            ("云端收入占比", "26.3%", "命门: '平台化'叙事的可验证指标", "中报分部", "停滞 <28% → 政企外包估值回归(P/S 3-12x)"),
        ],
    },
})
