...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "tracking": {
        ...
        "rows": [
            ("__band__", "一、核心驱动链"),
            ("AI 收入强度", "(示意)", "命门: AI 收入 = capex × 强度", "季报/行业 tracker(示意)", "强度回落 → 下调假设重算(示意)"),
            ("__band__", "二、商品周期"),
            ("商品合约价 + 库存", "(示意)", "命门: 价格路径与 normalize 时点", "月度合约价 tracker(示意)", "连续转跌 → 前移见顶年(示意)"),
            ("__band__", "三、需求总盘子"),
            ("AI capex 指引", "(示意)", "命门: 物理锚盘子", "hyperscaler 季报(示意)", "下修 >10% → 全链重算(示意)"),
        ],
    },
})
