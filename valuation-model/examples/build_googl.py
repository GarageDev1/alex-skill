...
def build():
    ...
    ma = K.write_multiple_assumptions(wb.create_sheet(S_MULT), {
        ...
        "why_text": ("Alphabet 穿越周期的耐用资产是『搜索的信息分发垄断 + AI 全栈（Gemini+TPU+Cloud）的可持续盈利』，"
                     ...
                     "因此主估值分母选核心经营 EPS，P/B 仅体检。若 AI 颠覆搜索货币化、或反垄断削弱数据飞轮，镜头才应向更保守倍数下移。"
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "tracking": {
            ...
            "rows": [
                ("__band__", "一、利润核心"),
                ("Search 收入增速", "Q1'26 +19%", "命门：AI 是否稀释搜索货币化", "季报 Search & other", "连续掉到个位数 → 转 Bear 看搜索"),
                ("__band__", "二、重估主线"),
                ("Cloud 增速/margin/backlog", "+63% / 32.9% / >$460B", "命门：Cloud 重估可持续性", "季报 Cloud segment", "增速/margin 不及 → 下调 cint/clopm"),
                ("__band__", "三、反垄断"),
                ("ad tech 救济裁决", "待裁定（2026 H2）", "命门：是否强制剥离 AdX", "DOJ/E.D.Va. 法院", "强制结构剥离 → Network/广告链重估"),
                ("__band__", "四、物理锚"),
                ("AIDC capex 指引", "2026E $830B 基座", "命门：AI 算力总盘子", "hyperscaler 季报", "下修 >10% → 全链重算"),
            ],
        },
    })
