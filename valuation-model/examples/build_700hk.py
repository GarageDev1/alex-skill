...
def build():
    ...
    px = K.write_price_chart(wb.create_sheet(S_PX), MONTHLY, {
        ...
        "rows": [("① 监管+反垄断深跌", "2021-2022 教培/游戏版号/反垄断 + 大股东减持,股价从 700+ 跌至 2022-10 低点约 190。"),
                 ...
                 ("④ AI 落后叙事杀估值", "2026 年内:DeepSeek V4 发布、元宝 C 端第四 → AI 落后叙事;AI 投入翻倍压利润+回购缩水+港股外资流出,跌破 4 万亿至 417。")],
    ...
    ma = K.write_multiple_assumptions(wb.create_sheet(S_MULT), {
        ...
        "why_text": ("腾讯穿越周期的耐用资产是『微信生态(14.3 亿 MAU + 关系链 + 支付闭环)+ 游戏 IP 矩阵 + 高毛利货币化能力』,不是账面净资产——它轻资产(相对盈利)、有定价权(游戏/广告)、生态锁定。"
                     ...
                     "镜头迁移触发:若 AI 颠覆微信入口、或游戏/广告定价权被侵蚀,镜头才应向更保守倍数下移。"),
        "why_rows": 6,
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "tracking": {
            ...
            "rows": [
                ("__band__", "一、增长核心(广告)"),
                ("广告 ARPU / 视频号加载率", "广告 +20%(1Q26) / 加载率 3-4%", "命门:加载率放量节奏 + AI eCPM", "季报营销服务 + 加载率口径", "加载率停滞/广告增速回落 → 转 Bear 看广告"),
                ("__band__", "二、利润(AI 投入)"),
                ("Non-IFRS OPM / AI 投入", "37%(2025) / AI 投入翻倍 >360 亿", "命门:AI 投入是否被货币化对冲", "季报 Non-IFRS 经营利润 + AI 拖累口径", "margin 持续回落 → 下调 opm/目标 P/E"),
                ("__band__", "三、估值与资金"),
                ("Non-IFRS forward P/E / 回购", "~14x / 连续 24 日回购", "命门:估值修复 + 回购支撑", "现价÷模型 EPS + 回购公告", "回购停 + 倍数破 12x → 转 Bear"),
                ("__band__", "四、安全垫"),
                ("投资组合公允价值", "1.04 万亿(2025末)", "命门:SOTP 下行保护", "季报上市/非上市投资公允价值", "组合大幅缩水 → 下调 SOTP 安全垫"),
            ],
        },
    })
