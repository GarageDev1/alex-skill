...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "final": {"band": "★ 综合判断(A+B+C+D 收成一句可执行的话)",
              "text": "回测 2025H2: 旋转关节合同+Optimus指引=强催化, 当时该看多✓(GAP由正转负那波已走完)。当下: 基本面 price in(Base≈现价)、GAP −15%(轻度透支)、催化待兑现、情绪过热后消化 → 持有(HOLD); 等 Optimus 量产证据(转Bull)或回落至核心价值~¥35(更好入场)。"},
    "tracking": {
        "intro": "哪个指标恶化 → 哪个假设先崩 → 触发什么动作。",
        "rows": [
            ("__band__", "一、机器人期权(最大摆动项)"),
            ("Optimus 量产/出货", "待验证", "命门: 机器人段收入拐点", "特斯拉季报/产能公告", "跳票→出货量下调→转 Bear"),
            ("三花机器人订单/收入", "~12亿合同", "命门: 单机价值×份额兑现", "公司公告/季报分部", "收入不确认→有效单机价值下调"),
            ("__band__", "二、汽车热管理"),
            ("汽车段增速", "2025 +9.1%", "命门: 单车价值量与份额", "季报分部 + NEV销量", "转负→有效单车价值下调"),
            ("__band__", "三、估值情绪"),
            ("forward P/E vs 历史带", "40x(26E) vs 28-36x", "命门: 情绪值档位", "股价/模型EPS", "突破50x=过热刹车; 跌破25x=Bear兑现"),
        ],
    },
})
