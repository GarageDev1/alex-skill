...
K.write_cover(wb.create_sheet(S_COVER), {
    ...
    "takeaways": [
        ...
        ("⑤ 主要风险", "FCBGA 客户放量证伪 / 折旧+利息压制 / 情绪退潮估值倍数下修 / 实控人减持与再融资摊薄。"),
    ],
})
...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "tracking": {
        ...
        "rows": [
            ("__band__", "一、载板爬坡主链"),
            ("载板分部毛利率(逐季)", "-16.1%(2025FY)", "关键敏感项: sopm 转正时点", "季报分部数据", "连续两季无改善 → 切 Bear 重算"),
            ("FCBGA 出货/稼动率", "小批量(~0.12 亿颗/年)", "关键敏感项: 物理锚爬坡曲线", "公告/调研纪要", "2026 末仍 <0.3 亿颗年化 → 下调 fcv"),
