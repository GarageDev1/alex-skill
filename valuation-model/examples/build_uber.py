...
def build():
    ...
    K.write_cover(wb.create_sheet(S_COVER), {
        ...
        "takeaways": [
            ...
            ("④ 估值判断", "Base 用 2027E 核心 EPS × 23x P/E。核心 EPS 口径下隐含价显著高于现价——市场用『AV 替代风险空头情景』定价（forward P/E 已压到历史下沿），是预期被错杀的中期预期差。"),
            ("⑤ 评级", "BUY（买入）：robotaxi 被市场误读为替代风险威胁、年内 -10% 被错杀；实际 Uber 是 AV 公司的需求聚合分发渠道(30+ 伙伴)，robotaxi 是免费期权。平台已规模化盈利(FCF $9.8B/转化 112%)。下行风险=AV 收敛到 1-2 家自营赢家。"),
        ],
    })
    ...
    px = K.write_price_chart(wb.create_sheet(S_PX), MONTHLY, {
        ...
        "rows": [("① 成长股估值倍数下修/盈利未证", "2022 加息周期杀无盈利成长股，叠加 Uber 自身尚未稳定盈利，股价探底 $20。"),
                 ...
                 ("④ robotaxi 替代风险恐慌错杀", "2025-11 起 Waymo/Tesla robotaxi 叙事发酵，市场担心 AV 去中介化，从高点回撤约 28%、年内约 -10% 至 ~$72。")],
    ...
    K.write_consensus(wb.create_sheet(S_CONS), {
        ...
        "overview": "约 51 家覆盖，Buy/Overweight 为主（46 buy / 5-8 hold / 1 sell），一致目标价均值约 $104.48（区间 $70-$150），隐含上行约 +45%。核心叙事：robotaxi 是看涨期权而非替代风险、平台盈利兑现、广告/会员高毛利、强 FCF + 回购、估值便宜。最大分歧：robotaxi 对 Uber 是期权还是结构性杀手。",
        "assumptions": [
    ...
    hm = K.write_hist_multiples(wb.create_sheet(S_HMULT), {
        ...
        "intro": "Uber 历史报表 P/E 早年无意义（2021/22 亏损）；2024-25 报表 P/E 因一次性 DTA 释放虚低。当下 TTM 报表 P/E ~17.9x（headline 假便宜），核心 EPS 口径下 ~30x。本轮折价由 robotaxi 替代风险恐慌驱动（forward P/E 压到历史下沿）。主线以核心 EPS × P/E 资本化盈利，P/B 不适用。",
    ...
    cmp = K.write_comparison(wb.create_sheet(S_CMP), {
        ...
        "summary": {
            ...
            "concl": "Base 2027E 核心 EPS×23x 隐含价显著高于现价 → 市场用『AV 替代风险空头情景』给 Uber 定价（forward P/E 压到历史下沿）。Bull 靠 robotaxi 期权证实 + 广告/会员加速 + 倍数扩张给更大上行；Bear 靠 AV 去中介化 + take rate 压缩给下行。风险收益偏多 → BUY。",
        },
    })
    ...
    K.write_dashboard(wb.create_sheet(S_DASH), {
        ...
        "blocks": [
            ...
            {"title": "D. 综合判断", "rows": [
                ("一句话结论", "BUY：robotaxi 被市场误读为替代风险威胁、年内 -10% 错杀；实际 Uber 是 AV 需求聚合分发层，robotaxi 是免费期权。平台已规模化盈利。", "上行看 robotaxi 期权 + 广告/会员；下行看 AV 收敛到自营赢家。", True),
            ]},
        ],
        "final": {"band": "最终判断",
                  "text": "Uber 的 alpha 问题：市场把 robotaxi 当成 Uber 的潜在替代者，2025-11 起从高点回撤约 28%、年内约 -10%。本模型用核心经营 EPS（剔除一次性 DTA 释放）下，现价对应的 forward P/E 已压到历史下沿——市场在用『AV 去中介化空头情景』给一家已规模化盈利、FCF $9.8B/转化 112%、与 30+ AV 公司合作的需求聚合平台定价。Base 2027E 隐含价显著高于现价 → 预期被错杀的中期预期差。最大未证伪下行是 robotaxi 收敛到 1-2 家自营赢家(Waymo/Tesla)绕过 Uber。评级 BUY。"},
        "tracking": {
            "intro": "投后跟踪按季报滚动更新。",
            "rows": [
                ("__band__", "一、物理锚"),
