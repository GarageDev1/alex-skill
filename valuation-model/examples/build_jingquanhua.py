# -*- coding: utf-8 -*-
"""
build_jingquanhua.py — 京泉华(002885.SZ)端到端公式联动估值模型(单币种 CNY·亿; P/E 主线 + P/B 平行镜头)。
...
K.write_consensus(wb.create_sheet(S_CONS), {
    ...
    "overview": "关键事实:京泉华无卖方研报覆盖、无 2026/2027/2028E 一致预期(东财研报接口=0 篇、同花顺无机构覆盖)。市场叙事来自散户/财经号(东财财富号)+ 互动易,不是机构定价。下表把这些『市场口径』当软信号对账,我们模型自行设定假设、标注与口径差异。",
    "assumptions": [
...
seg = K.write_segment_model(wb.create_sheet(S_SEG), {
    ...
    "groups": [
        ...
        ("SST 段 = 市场 × 京泉华份额", [
            ("SST/AI 收入(亿元)", None, K.N1, "历史=年报小试口径(2025≈0.4 亿); 前瞻=『情景切换』当前案 SST 收入行。这是 thesis 的关键敏感项。"),
...
dash = K.write_dashboard(wb.create_sheet(S_DASH), {
    ...
    "tracking": {
        ...
        "rows": [
            ("__band__", "一、SST 期权(关键敏感项)"),
            ("伊顿验证 + SST 收入", "待(2026 H1)", "关键敏感项: 全部估值溢价靠它", "互动易/季报/公告", "验证失败→转 Bear 重算(打回 45x PE)"),
            ("磁件分部增速", "2026 半年报看", "关键敏感项: SST 上量的财报印证", "半年报/三季报分部数据", "无跳升→下调 SST 收入假设"),
            ("__band__", "二、基础业务"),
            ("海外业务毛利率", "2025=25.66%", "关键敏感项: 基础业务净利率地板", "年报/半年报分地区", "回落→下调基础净利率(向 Bear)"),
            ("__band__", "三、情绪/资金"),
            ("两融余额 + 换手", "4.05 亿(+35%/周)", "关键敏感项: 情绪档位/股价支撑", "东财融资融券日更", "见顶回落→情绪值下调"),
        ],
    },
})
