...
### 强度反推的三项要求
1. **历史强度必须由实际数据反推**:每个 AI 直驱段的历史列使用 `历史段收入$B ÷ 当年 capex$B` Excel 公式,2025 年(最近实际年)的计算结果作为锚。前瞻值从该锚平滑延伸。过往曾出现历史反推 4.1%、前瞻直接填入 6.0% 的断层。前瞻第一年与锚的差异必须有 build-up 解释(如"该段约翻倍 > capex 1.7x 增速→强度小升")。
