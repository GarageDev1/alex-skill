#!/usr/bin/env python3
"""
validate_valuation.py — valuation-model 业务级校验(对应 SKILL.md 连通性铁律 + 验收清单)。
Excel 由 examples/build_kit.py + build_template.py 创建,公式重算由 scripts/recalc.py 完成。

检查(prose 铁律的机检化, 见 SKILL.md 沉淀协议):
  1 连通性(到物理锚)  2 孤儿  3 溯源覆盖  4 ≥2 镜头  5 禁单元格批注
  6 封面存在(05 §0)  7 表序=只向前引用 DAG(05 §1)
  8 估值对比防污染(04 三情景: 不得引『情景切换』当前案行/开关格)
  9 每情景 block 有隐含 forward P/E 行(04: P/B 给答案 P/E 验答案)
  10 估值对比历史列内置回测 ≈0%(04: 链放回过去能复现实际年末价; 需先 recalc 出缓存值)
不是公式重算(先走本仓 scripts/recalc.py)。

用法:  python validate_valuation.py <path.xlsx>
输出:  JSON {checks:[{name,status,detail}], summary, verdict}
       status ∈ pass / warn / fail。fail = 触碰铁律,必须修。
...
# 镜头识别用「明确 token」,避免 'pe' 命中 capex 这类误判
LENS_PATTERNS = {
...
# 锚/测算/假设——"算了没人用"才是要抓的病。展示表的链路质量由 连通性/防污染/回测 检查兜)
...
def main(path):
    ...
    # 连通性按「行级」判定——隐含价行的历史列是回测(锚定历史实际, 不该挂前瞻锚),
    # 只要该行有 ≥1 个公式格(前瞻列)能回溯到锚, 整行算通。
    ...
    if not anchor_sheets:
        checks.append({"name": "连通性(到物理锚)", "status": "warn",
                       "detail": "未找到 [ANCHOR] 标记或锚/基座 sheet;无法自动判定,请人工跑'改底层假设'测试"})
        ...
        checks.append({"name": "连通性(到物理锚)", "status": status,
                       "detail": (f"{reached_rows}/{len(out_rows)} 个隐含价输出行能回溯到锚 sheet "
                                  f"{sorted(anchor_sheets)}(行级: 历史回测列不要求挂锚; 识别方式:{src})"
                                  + (f";未通行 {fail_rows[:6]}" if fail_rows else ""))})

    # ── 5: 孤儿扫描(中间 driver 算了没人引用)──────────
    out_set = set(out_targets)
    # 行级判定: 一行里只要有 ≥1 个公式格被下游引用, 整行算"有人用"
    # (如情景切换『当前案』行: 前瞻列被链引用、历史列只作展示对照, 不算孤儿)
    ...
    if sw_sheet:
        if not cmp_sheet:
            ...
            wcmp = wb[cmp_sheet]
            # 8: 防污染 — 对比表不得引『情景切换』的 当前案行/开关与案序号所在行
            wsw = wb[sw_sheet]
            polluted_rows = set()
            ...
            checks.append({"name": "估值对比防污染",
                           "status": "pass" if not bad else "fail",
                           "detail": "case 列未引当前案/开关行" if not bad
                           else f"{len(bad)} 处引用当前案/开关行(抽样 {bad[:6]}) — 改引矩阵行"})
    ...
    # 铁律: 营收/利润/EBITDA/权益 等财务量纲行必须统一用 USD $B;
    ...
    # 因 mtext 合并吞掉 WACC 单元格 → 全链 #DIV/0! 却 keyword 仍 pass)。这一关补上。
    ...
    if wbv is not None:
        ...
        err_detail = ("无公式错误值" if not err_cells
                      else f"{len(err_cells)} 个单元格是公式错误值(抽样 {err_cells[:6]})——有公式/镜头算坏了,必须修")
    else:
