...
调用方提供已经校验的 data dict，本模块负责公司无关的工作表结构、跨表引用和布局。

标准 sheet 架构(05-excel-format §1, 只向前引用的 DAG):
  封面 → 历史财务与估值 → 股价走势 → 卖方研报共识 → 历史估值倍数 → 估值倍数假设
  → 情景切换 → 业务驱动量[ANCHOR] → 分部测算 → 利润与收入假设 → 情景估值 → 估值对比
  → 综合判断仪表盘
...
本文件不含公司专属数字。通用入口见 `examples/build_template.py`。
"""
from openpyxl.chart import LineChart, Reference
from openpyxl.worksheet.datavalidation import DataValidation

from .styles import *
from .styles import _esc
from .currency import uy
from .scenarios import nested_if_switch

# ════════════════════════════ sheet 结构函数 ════════════════════════════
# 约定:所有 write_* 吃一个已 create 好的 worksheet + data dict,就地写入,
# 返回"行号基准 dict"(供下游 sheet 跨表引用)。调用顺序 = DAG 顺序(见文件头)。
...
def write_history(ws, data):
    ...
    data 关键字段:
      ...
      band_note / notes [(行基准名 or 行号, 文本), ...](备注/来源可见列)
    ...
    band(ws, r, "市场估值 (历史=年末价→实际P/E·P/B; 当下列=TTM基准)", span); r += 1
    HPX = r
...
def write_hist_multiples(ws, data):
    """sheet 4 — 历史估值倍数(历史数据):自身逐年带+年内高低+当下 + 同业对照 + 相对核心同行比值。
...
def write_multiple_assumptions(ws, data):
    ...
    ①为什么主要估值方法(业务判断长文) ②三层分解(峰值×结构溢价×情绪值)→三案目标倍数矩阵
    ③卖方对账 ④数据源。三案倍数在此设置;『情景切换』引用并切换, 『情景估值』应用。

    data: title / intro / why_text(估值方法选择业务判断, 04 §估值方法选择) / why_rows(merge 行数, 默认8)
    ...
    band(ws, r, "① 为什么这个估值方法做主要方法 — 估值方法选择是业务判断(任何情景下辅助估值方法都要 double check)", 11); r += 1
    r = mtext(ws, r, data["why_text"], "K", data.get("why_rows", 8))

    band(ws, r, "② 三层分解 → 三案目标倍数(蓝字=情绪值; 三案在此设置, 『情景切换』负责切换)", 11); r += 1
    ...
    logic(ws, f"{L}{r}", data.get("sent_note", "情绪值=周期/情绪位置。1.0=达到『峰值×溢价』; >1=FOMO 情绪溢价; <1=下行折价。历史列=实际倍数÷(峰值×溢价)反推, 三案同值(一致性检验可见)。"))
    r += 1
    MSENT = r
...
def write_scenario_switch(ws, data):
    ...
    整条明细计算关系切换。矩阵布局: 参数名独立组标题行(指标本体描述) + 缩进三案行(各案故事,
    含历史实际列) + 当前案行。
    ...
    def _lever(lv):
        ...
        for i, cs in enumerate(cases):
            ...
            swr += 1
        lab(ws, f"A{swr}", "  当前案(引用此行)", note=True)
...
def write_anchor(ws, data):
    """sheet 7 — 业务驱动量基座([ANCHOR] 标记必在 A1)。
...
def write_segment_model(ws, data):
    ...
    用返回的 row map 回填(各公司计算关系不同, 不强行模板化)。
    ★ 建模方法见 anchors/<类型>.md(算力链 = AI 直驱段由 capex 驱动×强度 + 消费段走周期;
      强度以最近实际年为基准反推、历史列公式反推、早年失真标 n.m.)。
...
def write_scenario_valuation(ws, data):
    """sheet 10 — 情景估值(当前案逐年):主要估值方法(目标倍数×前瞻每股→隐含价) +
    ...
    band(ws, r, "主要估值方法(历史=实际年末价反推; 前瞻=目标倍数(当前案)×前瞻每股→隐含价)", 11); r += 1
    VPB = r
    ...
    r += 1
    VPBP = r
    lab(ws, f"A{r}", "隐含股价 主要估值方法(本币)", b=True); ws[f"A{r}"].fill = OUT
    for c in FCf:
        fml(ws, f"{c}{r}", f"={c}{VPB}*{BPSr(c)}*{FXc(c)}", PX)
    r += 1
    band(ws, r, "P/E 交叉验证(辅助方法)(历史=实际; 前瞻=隐含价÷前瞻EPS=隐含 forward PE)", 11); r += 1
    VPE = r
    ...
    r += 1
    VMCKB = r
    lab(ws, f"A{r}", data.get("mcap_fwd_label", "市值 前瞻·主要方法(本币兆)"))
    for c in FCf:
    ...
    if data.get("mcap_usd_skip"):
        ...
        VMCU = r
        lab(ws, f"A{r}", "市值 ($B; 历史=实际, 前瞻=主要方法)", b=True)
        for c in HC:
        ...
        r += 1
    VUP = r
    lab(ws, f"A{r}", "前瞻隐含价(主要方法) vs 当下股价", b=True)
    for c in FCf:
    ...
    pl = data.get("pe_lens")
    if pl:
        band(ws, r, "P/E 辅助估值方法(两种估值方法标的: 隐含价 = 目标P/E(当前案) × 前瞻EPS, 与主要方法并列)", 11); r += 1
        PETR = r
        ...
        r += 1
        PEPX = r
        lab(ws, f"A{r}", "隐含股价 P/E估值方法(本币)", b=True); ws[f"A{r}"].fill = OUT
        for c in FCf:
            fml(ws, f"{c}{r}", f"={c}{PETR}*{EPSr(c)}*{FXc(c)}", PX)
        r += 1
        lab(ws, f"A{r}", pl.get("mcap_label", "市值 前瞻·P/E估值方法"))
        for c in FCf:
            fml(ws, f"{c}{r}", f"={c}{PEPX}*{SHF}/{MD1}", N0)
        r += 1
        lab(ws, f"A{r}", "前瞻隐含价(P/E估值方法) vs 当下股价", b=True)
        for c in FCf:
...
def write_comparison(ws, data):
    ...
    ★ 避免引用受当前情景影响要求(04): case 列只引『情景切换』矩阵行(各案行) + 计算关系中未切换行 + 静态历史基准,
      绝不引明细计算关系上会随开关变的格(含『当前案』行/开关格)。Base 列与明细计算关系同构(开关=Base 时一致)。
    ★ 每案 block 必有: 隐含价行(key='px') + 隐含 forward P/E 行(key='ipe', P/E 核对) +
    ...
    data: title / intro / case_names / all_cols / all_years / hist_cols / fwd_cols(不含基年) /
          ...
          rows [{key, label, fmt, bold, out(高亮), hist(fn(col,ci,A)->'=..'|('inp',v)|None),
                 fwd(fn(col,j,ci,A)->'=..')}, ...]   A=该案已写行基准(写行前已含本行 key)
...
def write_dashboard(ws, data):
    """sheet 12(末张)— 综合判断仪表盘:A 基本面拐点 / B 估值错位(前瞻判断) /
