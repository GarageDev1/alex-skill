...
description: 把 equity-research-obsidian 的定性研报,接成一个端到端通电的估值模型——从业务最底层的物理驱动量(物理锚)推到隐含股价,叠历史财务/股价底座,PE/PB/DCF 多镜头三角验证,输出机构级 Excel + 决策 memo,并回填同 ticker 首次覆盖报告的估值章节。本 skill 是估值数字的唯一真相源:报告、journal、决策都引用它的隐含价。适用:已有定性研报、要把业务和价格联动起来建估值模型;也被 equity-research-obsidian 的 Phase 3 按名内嵌调用。触发:给 ticker + "估值"/"建模"/"更新模型"/"接着研报做模型"。不适用:纯定性研究(用 equity-research-obsidian)、纯 DCF 模板(用 financial-analysis:dcf-model)。
---

> **语言**: 中文输出,专业术语保留英文 (DCF, TAM, P/E, P/B, ROE, EBITDA, HBM, CoWoS 等)。
> **读者**: 你 (Agent) 读完这份 + 对应 reference 后,应能为任意公司从研报接出一个端到端通电、可对回现实的估值 Excel。

# Valuation Model

这个 skill 只做一件事:**把"业务"和"价格"用一条端到端通电的链连起来,不许有孤岛。**

它接在 `initiating-coverage`(定性研报)之后:研报告诉你公司是干啥的,这个 skill 把它折成一个能算出隐含股价、并且能对回历史现实的模型。

---

## ★★★ 这个模型是干什么的:预测 + 指导投资,不是拟合现价、不是跟随共识

最容易跑偏、也最致命的一点(血泪):**模型的目的是"预测价格该是多少、指导买不买",不是让输出"看起来像现在的市场价",也不是复述卖方共识。** 时刻拿这把尺子校准自己:

- **不是拟合现价**:当下锚(现价 / TTM 倍数)只是 reality-check 的**观测点**(知道"我们现在在哪"),不是**预测的目标**。预测(目标倍数 × 前瞻每股 → 隐含价)**应当、而且往往会**和现价不同——那个差,就是投资信号(上行 / 下行空间)。**为了让隐含价"贴近现价"去回调假设 = 自废武功。**
- **不是跟随共识**:**不怕给出和市场主流不同的估值**。只要你拿得出**事实 + 逻辑链**,就可以拍一个非主流的数——这恰恰是 alpha 的来源(和市场不同、且你对了)。共识是参照(`卖方研报共识` sheet),不是答案。
- **把握度(核心就在拿捏这个度)**:
  - 拍**太保守** → 错过一只很有潜力的票(本该买、没买)。
  - 拍**太激进** → 买了就亏。
  - 所以要在**事实 / 逻辑链支撑的范围内**给一个**有判断力**的数,不偏不倚。**最新数据(如最新季报)若指向比共识、比现价更高或更低的价值,就照事实修**——别因为"和市场不一样""和现价不一样"就缩手。
- **抽象出可跟踪指标 + 预测→跟踪→修正 闭环(活模型)**:模型的终极用途是**抽象出几个可观测的关键指标**(`综合判断仪表盘` sheet:基本面拐点+估值错位+催化剂+情绪确认,见 `05` 4c),当这些指标变化时,**回来更新模型数字、重新预测**。这是"指导投资的活模型",不是一次性拟合过去的死表。
...
- **不许有孤岛 sheet**:算了一堆数字却没有任何下游公式引用 = 孤儿,删掉或接上。
- **不许并行抄外部盈利截断链路**:不能营收模型辛苦建完,估值却直接硬塞一个卖方净利润,把链截断。
- **验收测试(就测这一条)**:改最底层的假设(如 AIDC 装机 GW / 出货量),隐含股价**必须跟着动**。不动 = 没通电 = 不合格。

> 这条铁律是为了治旧 coverage-model 的真实翻车:AIDC 拆解 / 营收预测 / PB 估值 三张表互不通电,价格架在卖方盈利上,业务模型成了摆设。详见 `references/03-modeling.md`。
