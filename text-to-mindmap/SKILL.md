...
description: 把一段文本/内容做成一张精美的竖向逻辑树思维导图图片(PNG)。当用户说"把这段内容做成思维导图""文本转思维导图""生成一张思维导图""帮我画个思维导图""整理成思维导图图片""做成 mindmap/逻辑树图",或给一段资料/大纲/文章要求可视化成思维导图时使用。输入是文本:给结构化大纲就直接用,给原始散文/长文就先生成结构化大纲再出图;若素材是一张图片,先把内容读成文本再走本流程。产出一张单图 PNG,固定竖向逻辑树布局(主干→彩色分类节点→横线+圆点→内容块),支持表格、象限散点图、硬件/要素网格等节点,内置深蓝商务/浅色杂志/科技暗色三套美术主题,分辨率可控。不适用于:可编辑的 XMind/FreeMind 文件、放射状/横向布局、纯文字大纲(不出图)。
---

# text-to-mindmap

把**文本内容**转成一张**精美的思维导图 PNG**。核心价值:一套固定的"竖向逻辑树"设计系统 + 确定性的 `JSON → HTML → 截图` 渲染管线,让同一种高质量观感可以一键复现,不靠每次手搓 HTML。
